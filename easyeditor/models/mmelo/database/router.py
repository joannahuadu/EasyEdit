...
class Router:
    ...
    def init_image_encoder(self):
        if self.config.image_encoder_name == 'dino':
            ...
            encoder = torch.hub.load('facebookresearch/dinov2:main', 'dinov2_vitl14_lc', source='github', trust_repo=True)
            encoder = encoder.to(self.config.device)

        elif self.config.image_encoder_name == 'clip':
            from transformers import CLIPProcessor, CLIPModel
            encoder = CLIPModel.from_pretrained("openai/clip-vit-base-patch32", cache_dir=self.config.model.cache_dir)
            processor = CLIPProcessor.from_pretrained("openai/clip-vit-base-patch32", cache_dir=self.config.model.cache_dir)
        elif self.config.image_encoder_name == 'vit':
            from transformers import ViTFeatureExtractor, AutoModel
            processor = ViTFeatureExtractor.from_pretrained('openai/clip-vit-base-patch32', cache_dir=self.config.model.cache_dir)
            encoder = AutoModel.from_pretrained('openai/clip-vit-base-patch32', cache_dir=self.config.model.cache_dir)
        else:
    ...
    def init_flag_embedding(self):
        ...
        encoder = FlagModel(self.config.text_encoder_name,
                            query_instruction_for_retrieval='Represent this sentence for searching relevant passages:')
        return encoder

    def batch_embed(self, batch):
        if self.config.task=='vqa' :
            image, text_input = batch["ori_image"], batch["prompt"]
        else:
            image, text_input = batch["ori_image"], batch["prompt"]
